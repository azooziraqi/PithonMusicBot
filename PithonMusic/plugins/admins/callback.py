...
from PithonMusic import YouTube, app
from PithonMusic.core.call import Pithon
from PithonMusic.core.channel import Pithon
from PithonMusic.misc import SUDOERS, db
from PithonMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from PithonMusic.utils.decorators.language import languageCB
from PithonMusic.utils.formatters import seconds_to_min
from PithonMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Pithon.pause_stream(chat_id)
        ...
        await Pithon.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Pithon.stop_stream(chat_id)
        ...
        await Pithon.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Pithon.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Pithon.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Pithon.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Pithon.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
