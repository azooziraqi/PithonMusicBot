...
from pytgcalls.exceptions import (AlreadyJoinedError,NoActivechannelCall,TelegramServerError)
from pytgcalls.types import (JoinedchannelCallParticipant,
                             LeftchannelCallParticipant, Update)
from pytgcalls.types.input_stream import AudioPiped, AudioVideoPiped
from pytgcalls.types.stream import StreamAudioEnded

import config
from strings import get_string
from PithonMusic import LOGGER, YouTube, app
from PithonMusic.misc import db
from PithonMusic.utils.database import (add_active_chat,
                                       add_active_video_chat,
                                       get_assistant,
                                       get_audio_bitrate, get_lang,
                                       get_loop, get_video_bitrate,
                                       channel_assistant, is_autoend,
                                       music_on, mute_off,
                                       remove_active_chat,
                                       remove_active_video_chat,
                                       set_loop)
from PithonMusic.utils.exceptions import AssistantErr
from PithonMusic.utils.inline.play import (stream_markup,
                                          telegram_markup)
from PithonMusic.utils.stream.autoclear import auto_clean
from PithonMusic.utils.thumbnails import gen_thumb
...
class Call(PyTgCalls):
    ...
    async def pause_stream(self, chat_id: int):
        assistant = await channel_assistant(self, chat_id)
    ...
    async def stop_stream(self, chat_id: int):
        ...
        try:
            ...
            await assistant.leave_channel_call(chat_id)
        except:
            pass
    ...
    async def stream_call(self, link):
        assistant = await channel_assistant(self, config.LOG_CHANNEL_ID)
        await assistant.join_channel_call(
            config.LOG_channel_ID,
            AudioVideoPiped(link),
            stream_type=StreamType().pulse_stream,
        )
        await asyncio.sleep(0.5)
        await assistant.leave_channel_call(config.LOG_channel_ID)
    ...
    async def join_call(
        ...
        try:
            ...
            try:
                ...
                raise AssistantErr(
                    "**No Active Voice Chat Found**\n\nPlease make sure channel's voice chat is enabled. If already enabled, please end it and start fresh voice chat again and if the problem continues, try /restart"
                )
    ...
    async def change_stream(self, client, chat_id):
        ...
        try:
            ...
            if not check:
                await _clear_(chat_id)
                return await client.leave_channel_call(chat_id)
        except:
            try:
                await _clear_(chat_id)
                return await client.leave_channel_call(chat_id)
    ...
    async def decorators(self):
        ...
        async def participants_change_handler(client, update: Update):
            if not isinstance(
                update, JoinedchannelCallParticipant
            ) and not isinstance(update, LeftchannelCallParticipant):
                return
            ...
            if not users:
                ...
                final = (
                    users + 1
                    if isinstance(update, JoinedchannelCallParticipant)
                    else users - 1
                )
